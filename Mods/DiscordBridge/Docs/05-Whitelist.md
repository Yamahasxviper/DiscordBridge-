...
All whitelist settings live in their own dedicated file:
...
> **Backup:** the mod writes `<ServerRoot>/FactoryGame/Saved/Config/Whitelist.ini` on
> every server start.  If a mod update resets `DefaultWhitelist.ini`, the backup is
...
The Discord role ID whose members are allowed to run `!whitelist` management commands.
...
The Discord role ID used to identify whitelisted members.
...
The Discord channel ID of a dedicated channel for whitelisted members.
