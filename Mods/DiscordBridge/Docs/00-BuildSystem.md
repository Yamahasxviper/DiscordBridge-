...
| `Plugin 'EOSShared' … does not contain the 'EOSShared' module` (FactoryEditor IntelliSense) | `ReliableMessaging → SocketSubsystemEOS → EOSShared` dependency chain enables `SocketSubsystemEOS` for the Editor target; CSS UE does not ship the `EOSShared` module for Editor or dedicated-server targets | Fixed in `FactoryGame.uproject` (`SocketSubsystemEOS` denied for `Server` and `Editor`) and `CSSCompatStubs` provides a `SocketSubsystemEOS` stub module for those targets so `ReliableMessagingEOSP2P` can resolve the dependency |
