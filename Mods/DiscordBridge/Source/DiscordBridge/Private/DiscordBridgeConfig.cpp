...
FDiscordBridgeConfig FDiscordBridgeConfig::LoadOrCreate()
{
	...
	if (!bLoadedFromMod)
	{
		if (!PlatformFile.FileExists(*ModFilePath))
		{
			...
			TEXT("UseEmbedsForPlayerEvents=False\n")
			TEXT("# When True, adds thumbs-up/thumbs-down reactions to player-join messages. Default: False\n")
			TEXT("EnableJoinReactionVoting=False\n")
			TEXT("# Number of thumbs-down reactions needed to auto-kick. 0 = disabled. Default: 0\n")
			...
			TEXT("# list on server start. Users can also type /help.\n")
			TEXT("# Leave empty to disable the automatic post (but /help still works).\n")
			...
			TEXT("# Message sent privately in-game to a player when they join the server.\n")
			TEXT("# Use this to advertise available commands (/discord, /verify, /commands).\n")
			TEXT("# Leave empty to disable (default).\n")
			TEXT("# Placeholder: %PlayerName%\n")
			TEXT("JoinHintMessage=\n")
			TEXT("\n")
			TEXT("# -- ON-JOIN IN-GAME BROADCAST -----------------------------------------------\n")
			TEXT("# Set to False to disable the broadcast without clearing the message text.\n")
			TEXT("# Default: True (enabled)\n")
			TEXT("InGameJoinBroadcastEnabled=True\n")
