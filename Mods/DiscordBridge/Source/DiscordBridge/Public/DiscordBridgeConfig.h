...
struct DISCORDBRIDGE_API FDiscordBridgeConfig
{
	...
	// ── BanSystem notifications ───────────────────────────────────────────────

	/**
	 * Master on/off switch for BanSystem ban/unban event notifications.
	 * When true, DiscordBridge posts a Discord message whenever a Steam or EOS
	 * ban is added or removed from any source (Discord commands, in-game chat
	 * commands, or any other mod/script that calls the ban API).
	 * Default: false (disabled).
	 *
	 * Requires BanSystem to be installed alongside DiscordBridge.
	 */
	bool bBanNotificationsEnabled{ false };

	/**
	 * Snowflake ID of the Discord channel where ban/unban event notifications
	 * are posted.  Leave empty to fall back to the main bridged channel (ChannelId).
	 *
	 * Use a dedicated admin channel to keep ban activity separate from regular chat.
	...
	/**
	 * Message posted to Discord when a Steam ban is issued (from any source).
	 * Available placeholders:
	 *   %PlayerId%  – Steam64 ID of the banned player
	 *   %Reason%    – ban reason string
	 *   %BannedBy%  – name of the admin who issued the ban
	 *   %Expiry%    – "Permanent" or UTC date-time when the ban expires
	 * Leave empty to disable Steam ban notifications.
	 */
	FString SteamBanNotificationMessage{
		TEXT(":hammer: **BanSystem** — Steam player `%PlayerId%` banned by **%BannedBy%** "
		     "(%Expiry%) — Reason: %Reason%")
	};

	/**
	 * Message posted to Discord when a Steam ban is removed (from any source).
	 * Available placeholder:
	 *   %PlayerId%  – Steam64 ID of the unbanned player
	 * Leave empty to disable Steam unban notifications.
	 */
	FString SteamUnbanNotificationMessage{
		TEXT(":white_check_mark: **BanSystem** — Steam player `%PlayerId%` was unbanned.")
	};

	/**
	 * Message posted to Discord when an EOS ban is issued (from any source).
	 * Available placeholders:
	 *   %PlayerId%  – EOS Product User ID of the banned player
	 *   %Reason%    – ban reason string
	 *   %BannedBy%  – name of the admin who issued the ban
	 *   %Expiry%    – "Permanent" or UTC date-time when the ban expires
	 * Leave empty to disable EOS ban notifications.
	 */
	FString EOSBanNotificationMessage{
		TEXT(":hammer: **BanSystem** — EOS player `%PlayerId%` banned by **%BannedBy%** "
		     "(%Expiry%) — Reason: %Reason%")
	};

	/**
	 * Message posted to Discord when an EOS ban is removed (from any source).
	 * Available placeholder:
	 *   %PlayerId%  – EOS Product User ID of the unbanned player
	 * Leave empty to disable EOS unban notifications.
	 */
	FString EOSUnbanNotificationMessage{
