...
// OpenSSL headers — available and needed on all three Alpakit build targets:
...
// scope.  OpenSSL's ossl_typ.h (line 144) also declares `typedef struct ui_st UI`
// at global scope.  This produces error C2365 ("redefinition; previous definition
// was 'namespace'") because a C++ namespace and a typedef cannot share the same
...
// ─────────────────────────────────────────────────────────────────────────────
// OpenSSL helpers — compiled on Win64 and Linux (all three Alpakit targets:
//   "Windows" / "WindowsServer" / "LinuxServer")
// ─────────────────────────────────────────────────────────────────────────────
#if PLATFORM_WINDOWS || PLATFORM_LINUX

// Returns the topmost error string from the OpenSSL error queue, or an empty
// string when the queue is empty.  Drains one entry so subsequent calls report
// subsequent errors rather than repeating the same one.
static FString GetOpenSSLErrorString()
{
	...
	char ErrBuf[256];
	ERR_error_string_n(ErrCode, ErrBuf, static_cast<int>(sizeof(ErrBuf)));
	return FString(ANSI_TO_TCHAR(ErrBuf));
}
...
bool FSMLWebSocketRunnable::InitSslContext()
{
	...
	if (!SslCtx)
	{
		const FString SslErr0 = GetOpenSSLErrorString();
		UE_LOG(LogSMLWebSocket, Error, TEXT("SMLWebSocket: SSL_CTX_new failed%s%s"),
		       SslErr0.IsEmpty() ? TEXT("") : TEXT(": "), SslErr0.IsEmpty() ? TEXT("") : *SslErr0);
		return false;
	}
	...
	if (!SslInstance)
	{
		const FString SslErr1 = GetOpenSSLErrorString();
		UE_LOG(LogSMLWebSocket, Error, TEXT("SMLWebSocket: SSL_new failed%s%s"),
		       SslErr1.IsEmpty() ? TEXT("") : TEXT(": "), SslErr1.IsEmpty() ? TEXT("") : *SslErr1);
		return false;
	}
	...
	if (!ReadBio || !WriteBio)
	{
		const FString SslErr2 = GetOpenSSLErrorString();
		UE_LOG(LogSMLWebSocket, Error, TEXT("SMLWebSocket: Failed to create memory BIOs%s%s"),
		       SslErr2.IsEmpty() ? TEXT("") : TEXT(": "), SslErr2.IsEmpty() ? TEXT("") : *SslErr2);
		return false;
	}
...
bool FSMLWebSocketRunnable::PerformSslHandshake(const FString& Host)
{
	...
	for (;;)
	{
		...
		{
			const FString SslErrHs = GetOpenSSLErrorString();
			UE_LOG(LogSMLWebSocket, Error, TEXT("SMLWebSocket: SSL handshake error %d%s%s"),
			       Err, SslErrHs.IsEmpty() ? TEXT("") : TEXT(": "), SslErrHs.IsEmpty() ? TEXT("") : *SslErrHs);
			return false;
		}
	}
...
bool FSMLWebSocketRunnable::FeedSslReadBio()
{
	...
	{
		UE_LOG(LogSMLWebSocket, Warning,
		       TEXT("SMLWebSocket: FeedSslReadBio – TCP recv returned %d (connection closed or error)"),
		       BytesRead);
		return false;
	}
...
bool FSMLWebSocketRunnable::SslWrite(const uint8* Data, int32 DataSize)
{
	...
	while (Written < DataSize)
	{
		...
		else
		{
			...
			{
				const FString SslErrW = GetOpenSSLErrorString();
				UE_LOG(LogSMLWebSocket, Error, TEXT("SMLWebSocket: SSL_write error %d%s%s"),
				       Err, SslErrW.IsEmpty() ? TEXT("") : TEXT(": "), SslErrW.IsEmpty() ? TEXT("") : *SslErrW);
				return false;
			}
		}
	}
...
bool FSMLWebSocketRunnable::RawSend(const uint8* Data, int32 DataSize)
{
	...
	while (TotalSent < DataSize)
	{
		...
		{
			UE_LOG(LogSMLWebSocket, Warning,
			       TEXT("SMLWebSocket: RawSend failed (sent %d of %d bytes, last chunk returned %d)"),
			       TotalSent, DataSize, Sent);
			return false;
		}
		TotalSent += Sent;
	}
...
bool FSMLWebSocketRunnable::RawRecvExact(uint8* Buffer, int32 BytesRequired)
{
	...
	while (Total < BytesRequired)
	{
		...
		{
			UE_LOG(LogSMLWebSocket, Warning,
			       TEXT("SMLWebSocket: RawRecvExact failed (received %d of %d required bytes, last chunk returned %d)"),
			       Total, BytesRequired, Received);
			return false;
		}
		Total += Received;
	}
...
bool FSMLWebSocketRunnable::ReadHttpUpgradeResponse(const FString& ExpectedAcceptKey)
{
	...
	{
		UE_LOG(LogSMLWebSocket, Error, TEXT("SMLWebSocket: Server did not return 101 Switching Protocols.\n%s"), *ResponseHeaders);
		return false;
	}
...
bool FSMLWebSocketRunnable::ProcessIncomingFrame()
{
	...
	if (!NetRecvExact(Header, 2))
	{
		...
		{
			UE_LOG(LogSMLWebSocket, Warning,
			       TEXT("SMLWebSocket: ProcessIncomingFrame – failed to read 2-byte frame header "
			            "(connection lost or server closed the connection)"));
		}
		return false;
	}
...
FString FSMLWebSocketRunnable::GenerateWebSocketKey()
{
	...
	uint8 RawKey[16];
#if PLATFORM_WINDOWS || PLATFORM_LINUX
	// Use OpenSSL's CSPRNG when available (Win64 and Linux — all three Alpakit targets).
...
#if PLATFORM_WINDOWS || PLATFORM_LINUX
	// Use OpenSSL's SHA1 (available on Win64 and Linux — all three Alpakit targets).
