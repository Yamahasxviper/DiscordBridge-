...
// Guarded by PLATFORM_WINDOWS||PLATFORM_LINUX because OpenSSL is only linked on
// these platforms.  This covers all three Alpakit targets:
//   Windows      (Win64, TargetType.Game)   – listen-server hosts on Windows
...
private:
	...
	// ── OpenSSL helpers ───────────────────────────────────────────────────────
	// Compiled only when OpenSSL is linked (Win64 and Linux — all three
	// Alpakit targets: "Windows", "WindowsServer", and "LinuxServer").
#if PLATFORM_WINDOWS || PLATFORM_LINUX
	...
	// OpenSSL objects (only valid when bUseSsl == true, and only on Win64/Linux
	// where OpenSSL is linked — see the PLATFORM_WINDOWS||PLATFORM_LINUX guard above)
#if PLATFORM_WINDOWS || PLATFORM_LINUX
