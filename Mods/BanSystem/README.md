...
### Connection enforcement (dedicated servers)

Ban enforcement uses a **two-layer** approach to ensure banned players can
never enter the server:

1. **PreLogin hook (primary)** — fires *before* the connection handshake
   completes.  If the player's Steam64 ID or EOS PUID is on the ban list, the
   `ErrorMessage` is set to the kick reason and the connection is rejected
   immediately.  The player never enters the game world.

2. **PostLogin hook (fallback)** — fires *after* the player's
   `PlayerController` and `PlayerState` have been created.  This catches any
   edge case where the player's platform IDs were not yet available at PreLogin
   time (e.g. slow EOS authentication).  A banned player detected here is
   kicked via `GameSession::KickPlayer()`.

This dual-layer design guarantees that bans are enforced on dedicated servers
regardless of authentication timing.
...
2. The PUID is **not** directly available from `ToString()` on the
   ...
   `EOSId::GetProductUserId(const FUniqueNetIdRepl&, FString&)`, which handles
   both the V1 `FUniqueNetId` (OnlineSubsystemEOS) and V2 `FAccountId`
   (OnlineServicesEOSGS) representations used by the v2 API.
